...
class SoClimb:
    ...
    def write(self):
        ...
        for i in range(len(search_key)):
            result = self.climb_so(search_key[i])
            print(search_key[i])
            for c in range(len(result)):
                title = result[c].get('title')
                url = result[c].get('href')
                self.set_excel_data('search_result', c + 29, n, title)
                self.set_excel_data('search_result', c + 29, n + 1, url)
