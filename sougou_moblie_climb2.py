...
class SouGouMoblieClimb:
    ...
    def climb_sougou_moblie(self,key):
        ...
        requests.get('https://www.sogou.com', headers=headersParameters)
        ...
        if httpRsp.status_code != 200:
            ...
            if key =='英荔':
                ...
                for index in range(3, 3):
                    # 获取标题所在的a标签
                    # print(results[index])
                    div_a = results[index + 2].select("div>a")
                    if len(div_a) > 0:
                        aTag = div_a[0]
                    else:
                        aTag = results[index + 2].select("h3 a")[0]
                    #aTag = results[index].select("h3 a")[0]
                    # 获取标题的文本
                    title = aTag.get_text()
                    print(title)
            ...
            if key == '英荔商学院':
                soup = BeautifulSoup(httpRsp.text, "lxml")
                results = soup.select(".rb")
                print(len(results),1111111111111111)
    ...
    def write(self):
        ...
        for i in range(len(search_key)):
            result = self.climb_sougou_moblie(search_key[i])
            for c in range(len(result)):
                title = result[c].get('title')
                url = result[c].get('href')
                self.set_excel_data('search_result', c + 56, n, title)
                self.set_excel_data('search_result', c + 56, n + 1, url)
                if title == '首页 | 英荔商学院':
